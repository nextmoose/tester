			    ...
			    let
				  ...
				  let
				    visitor =
					  ...
					  in builtins.trace ( if observed == expected then "TRUE" else "success = ${ if success then "SUCCESS" else "NOT SUCCESS" } value = ${ if value then "VALUE" else "UNVALUED" }" ) ( observed == expected ) ;
