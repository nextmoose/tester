			    ...
			    let
				  ...
				  let
				    visitor =
					  ...
					  observed = builtins.tryEval ( observer-function ( builtins.getAttr system implementation.lib ) ;
					  in builtins.trace ( if success then "SUCCESS" else "NOT SUCCESS value ${ builtins.typeOf value } = ${ if value then "VALUED" else "UNVALUED" } observed.success = ${ if observed.success then "SUCCESS" else "FAILURE" } observed.value = ${ builtins.typeOf observed.value }" ) ( observed == expected ) ;
