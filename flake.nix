			    ...
			    let
				  ...
				  let
				    visitor =
					  ...
					  in builtins.trace ( if success then "SUCCESS" else "NOT SUCCESS value ${ builtins.typeOf value } = ${ if value then "VALUED" else "UNVALUED" } observed.success = ${ if observed.success then "SUCCESS" else "FAILURE" } observed.value = ${ if observed.value then "VALUED" else "UNVALUED" }" ) ( observed == expected ) ;
